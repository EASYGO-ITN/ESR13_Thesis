...
whps = np.logspace(math.log10(P_geo_in + 0.01e5), math.log10(P_geo_in + 20e5), 50)
P_geo_SH = P_geo_in + 21e5
...
for i, whp in enumerate(whps):
    ...
    water.update(cp.HmassP_INPUTS, H_geo_in, whp)
    T_geo_in = water.T()
    Ts_geo.append(T_geo_in)

    T_wf_evaps = np.linspace(T_wf_cond + 5, T_geo_in - 10, 20)
...
ax[2].plot(whps, Rs)
ax_twin2 = ax[2].twinx()
ax_twin2.plot(whps, Hs)
ax_twin2.plot(whps, Power)

# tikzplotlib.save("ORCPerf.tex")

...
wf = cp.AbstractState("?", "butane&Pentane")
wf.set_mole_fractions([0.5, 0.5])
...
# pump outlet
sol = root_scalar(Ssearch, (P_wf_evap, S_wf_in), method="secant", x0=T_wf_in, x1=T_wf_in+1)
...
sol = root_scalar(Hsearch, (P_wf_evap, H_wf_pump_out), method="secant", x0=T_wf_pump_isen, x1=T_wf_pump_isen+1)
...
T_PHE = list(np.linspace(T_wf_pump_out, T_wf_PHE_out, 50))
S_PHE = []
H_PHE = []

for i, t in enumerate(T_PHE):
    wf.update(cp.PT_INPUTS, P_wf_evap, t)
    S_PHE.append(wf.smass())
    H_PHE.append(wf.hmass())

# turbine outlet
sol = root_scalar(Ssearch, (P_wf_cond, S_wf_PHE_out), method="secant", x0=T_wf_PHE_out, x1=T_wf_PHE_out-1)
...
sol = root_scalar(Hsearch, (P_wf_cond, H_wf_turb_out), method="secant", x0=T_wf_turb_isen, x1=T_wf_turb_isen+1)
...
T_COND = list(np.linspace(T_wf_turb_out, T_wf_cond_out, 50))
S_COND = []
H_COND = []

for i, t in enumerate(T_COND):
    wf.update(cp.PT_INPUTS, P_wf_cond, t)
    S_COND.append(wf.smass())
    H_COND.append(wf.hmass())
...
ss = [S_wf_in/1000, S_wf_pump_out/1000] + [s/1000 for s in S_PHE] + [S_wf_PHE_out/1000, S_wf_turb_out/1000] + [s/1000 for s in S_COND]
ts = [T_wf_in, T_wf_pump_out] + T_PHE + [T_wf_PHE_out, T_wf_turb_out] + T_COND

ss_dots = [S_wf_in/1000, S_wf_pump_out/1000, S_wf_PHE_out/1000, S_wf_turb_out/1000, S_wf_cond_out/1000]
ts_dots = [T_wf_in, T_wf_pump_out, T_wf_PHE_out, T_wf_turb_out, T_wf_cond_out]
...
H_COND.reverse()
T_COND.reverse()
qs_out = [0] + [(h-H_wf_cond_out)/R/1000 for h in H_COND]
ts_out = [T_wf_cond_out] + T_COND

ax[1].plot([0, (H_wf_PHE_out-H_wf_pump_out)/R/1000], [T_geo_out, T_geo_in], label="Geofluid", color="#ff7f0e")
ax[1].plot(qs_in, ts_in, label="1:1 n-Butane \\& Pentane", color="#1f77b4")
ax[1].plot(qs_out, ts_out, color="#1f77b4")
ax[1].set_ylim(300, tmax)
# ax[1].set_xlim(0, 400)
...
ax_twin = ax[1].twinx()
ax_twin.set_ylim(300-273, tmax-273)
...
tikzplotlib.save("ORC_mixturePerf.tex")

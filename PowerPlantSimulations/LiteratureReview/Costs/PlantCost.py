...
for t in ts:
    ...
    ax.plot(ps, vX_ORC_sub, label="ORC T={:.0f}".format(t))

    vX_ORC_sub_spec = [vX.ORC_subcrit(t, p)/p for p in ps]
    bx.plot(ps, vX_ORC_sub_spec, label="ORC T={:.0f}".format(t))

for t in ts:
    vX_flash_sing = [vX.Flash_single(t, p) for p in ps]
    ax.plot(ps, vX_flash_sing, label="DSC T={:.0f}".format(t))

    vX_flash_sing_spec = [vX.Flash_single(t, p)/p for p in ps]
    bx.plot(ps, vX_flash_sing_spec, label="DSC T={:.0f}".format(t))

ax.plot(break_ps, break_Cs, "o-", color="red", label="Break-even")
bx.plot(break_ps, break_specCs, "o-", color="red", label="Break-even")
...
bx.set_ylabel("Specific Plant Cost/\\(\\$2022/\\unit{\\mega\\watt}")
bx.set_xscale("log")
bx.legend()

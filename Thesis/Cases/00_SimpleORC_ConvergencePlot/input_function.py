...
def init_cycle(Variables, Parameters):
    ...
    hot_fluid_stream.update("TQ", 200 + 273.15, 0.2)
    ...
    Pmax = P_crit * Variables[0]

    workingfluid.update("PQ", Pmax, 0.5)
    Tsat = workingfluid.properties.T
    Tmax = Tsat + Variables[1]

    _variables = [Pmax, Tmax, 308]

    # Cycle = simpleORC(recu=Parameters["is Cycle regenerated"])
    ...
    #
    # _variables = list(Variables) + [308]

    return Cycle, _variables

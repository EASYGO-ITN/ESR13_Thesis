...
Pmin = [[[np.NAN + 0 for q in qs] for t in ts] for f in fluids]
...
for result in results:
    if result:
        ...
        Pmin[f_id][t_id][q_id] = result["Pmin"] * 1e-5
...
# plot the minimum pressure
fig, axs = plt.subplots(nrows=len(fluids))
for j, fluid in enumerate(fluids):
    axs[j].set_title("{}".format(fluid[0]))

    f_id = fluids.index(fluid)
    for i, p in enumerate(Pmin[f_id]):
        label = "\\(T_{geo}^{in}=\\)\\qty{" + "{:.0f}".format(ts[i]) + "}{\\K}"
        axs[j].plot(qs, p, label=label, color=colors[f_id][i])

    axs[j].set_xlabel("Inlet Steam Quality/\\unit{\\percent}")
    axs[j].set_ylabel("Pmin/\\unit{\\bar}")

    # axs[j].set_ylim(0.25, 0.85)

axs[-1].legend()

tikzplotlib.save("Plots/Pmin.tex", figure=fig)



axs[-1].legend()

tikzplotlib.save("Plots/TmaxTmin.tex", figure=fig)

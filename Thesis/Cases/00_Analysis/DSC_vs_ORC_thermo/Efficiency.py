...
def get_DSC_results():
    ...
    eff_II_BF = [[np.NAN + 0 for q in qs] for t in ts]
    eff_II_FUNC = [[np.NAN + 0 for q in qs] for t in ts]
    ...
    for result in results:
        if result:
            ...
            eff_II_BF[t_id][q_id] = result["eta_II_BF"]*100
            eff_II_FUNC[t_id][q_id] = result["eta_II_FUNC"]*100

    qs = [q * 100 for q in qs]

    return eff_II_BF, eff_II_FUNC, ts, qs
...
def get_ORC_results():
    ...
    eff_II_BF = [[[np.NAN + 0 for q in qs] for t in ts] for f in fluids]
    eff_II_FUNC = [[[np.NAN + 0 for q in qs] for t in ts] for f in fluids]
    eff_II_BF_best = [[-1e15 + 0 for q in qs] for t in ts]
    eff_II_FUNC_best = [[-1e15 + 0 for q in qs] for t in ts]
    ...
    for result in results:
        if result:
            ...
            BF = result["eta_II_BF"]*100
            FUNC = result["eta_II_FUNC"]*100

            eff_II_BF[f_id][t_id][q_id] = BF
            eff_II_FUNC[f_id][t_id][q_id] = FUNC

            if BF > eff_II_BF_best[t_id][q_id]:
                eff_II_BF_best[t_id][q_id] = BF

            if FUNC > eff_II_FUNC_best[t_id][q_id]:
                eff_II_FUNC_best[t_id][q_id] = FUNC

    qs = [q * 100 for q in qs]

    return eff_II_BF_best, eff_II_FUNC_best, eff_II_BF, eff_II_FUNC, ts, qs, fluids

DSC_eff_II_BF, DSC_eff_II_FUNC, DSC_ts, DSC_qs = get_DSC_results()
ORC_eff_II_BF_best, ORC_eff_II_FUNC_best, ORC_eff_II_BF, ORC_eff_II_FUNC, ORC_ts, ORC_qs, ORC_fluids = get_ORC_results()
...
if __name__ == "__main__":
    ...
    for i, t in enumerate(ORC_eff_II_BF_best):
        ax[0].plot(ORC_qs, t, color=colors[0][i])

    for i, t in enumerate(DSC_eff_II_BF):
    ...
    ts_map =[]
    qs_map=[]
    for i, W in enumerate(DSC_eff_II_BF):
        W_DSC = np.array(W)
        W_ORC = np.array(ORC_eff_II_BF_best[i])
        qs = np.array(DSC_qs)
    ...
    w = gax.plot(qs_map, ts_map, label="BF")
    bx.plot(qs_map, ts_map, label="BF Parity")
    ...
    # spec costs
    for i, t in enumerate(ORC_eff_II_FUNC_best):
        ax[1].plot(ORC_qs, t, color=colors[0][i])

    for i, t in enumerate(DSC_eff_II_FUNC):
    ...
    ts_map =[]
    qs_map=[]
    for i, W in enumerate(DSC_eff_II_FUNC):
        W_DSC = np.array(W)
        W_ORC = np.array(ORC_eff_II_FUNC_best[i])
        qs = np.array(DSC_qs)
    ...
    q_max = [0 for q in qs_map]
    # gax.fill_betweenx(ts_map, qs_map, q_max, alpha=0.3)
    w = gax.plot(qs_map, ts_map, label="FUNC")
    cx.plot(qs_map, ts_map, label="FUNC Parity")
    ...
    tikzplotlib.save("Plots/EFF_II.tex")

...
def get_DSC_NCG():
    with open("../../15_DSC_single_flash_NCG_CarbFix_final_additional_cooling/sensitivity_results.json", "r") as file:
        results = json.load(file)
...
ax[0].plot(ORC_ncg, ORC_Wnet_exNCG, label="ORC - Power Plant")
ax[0].plot(DSC_ncg, DSC_Wnet_exNCG, label="DSC - Power Plant")
ax[0].plot(ORC_ncg, ORC_Wnet, label="ORC - CarbFix")
ax[0].plot(DSC_ncg, DSC_Wnet, label="DSC - CarbFix")
ax[0].set_xlabel("Geofluid \\ce{CO2} content/\\unit{\\mol\\percent}")
ax[0].set_ylabel("Net electrical power /\\unit{\\mega\\watt}")
ax[0].set_xlim(0, None)
ax[0].legend()

ax[1].plot(ORC_ncg, ORC_SpecCost, label="ORC")
ax[1].plot(DSC_ncg, DSC_SpecCost, label="DSC")

...
for i, dt in enumerate(DT_pinch_cond):
    label = "\\(T_{geo}^{in}=\\)\\qty{"+"{:.0f}".format(ts[i])+"}{\\K}"
    axs.plot([0, 1], [-1, -1], label=label, color=colors[3][i])
    axs.plot(qs, dt, color=colors[0][i])

axs.plot([0, 100], [5, 5], label="Thermodynamic Opt.")
axs.set_xlabel("Inlet Steam Quality/\\unit{\\percent}")
axs.set_ylabel("Condenser minimum approach temperature/\\unit{\\K}")
axs.set_ylim([0, 35])
axs.legend()

tikzplotlib.save("Plots/SingleFlash_DT_pinchs.tex")

...
class DirectCycle(Cycle):
    ...
    def update_mass_rate(self, m):
        ...
        if not self.ignore_liqefaction:
    ...
    def __calc_mass_rates(self, vapour, brine, ncg, condensate):
        ...
        if self.initial_repr_needed:
            self.condensate_pump_llP.update_inlet_rate(cond_m)
            self.ncg_comp_llp.update_inlet_rate(ncg_m)
        ...
        if self.NCG_handling:
            ...
            if not self.ignore_liqefaction:
                ncg_coolant_m = ncg_m * self.co2_liquefier.MassRatio
                self.co2_liquefier.update_inlet_rate(ncg_m, ncg_coolant_m)
                self.ncg_pump.update_inlet_rate(ncg_m)
    ...
    def __calc_performance(self):
        ...
        if self.initial_repr_needed:
            self.parasitic_power += self.condensate_pump_llP.work \
        ...
        if self.initial_repr_needed:
            self.parasitic_power_elec += self.condensate_pump_llP.power_elec \
    ...
    def __calc_cost(self):
        ...
        if self.initial_repr_needed:
            self.costs.append({"equip": "LLP_CondPump", "val": self.condensate_pump_llP.cost})
            self.costs.append({"equip": "LLP_NCGComp", "val": self.ncg_comp_llp.cost})
        if self.NCG_handling:
    ...
    def __calc_NCG_handling(self, brine_out, condensate_out, ncg_out):
        ...
        if brine_out.properties.P < self.geofluid_P_out:
            ...
            self.brine_pump.set_inputs(brine_out, brine_out.properties.P*1.01)
            brine_hp = self.brine_pump.calc()

...
class ORC(BinaryCycle):
    ...
    def __calc_cost(self):
        ...
        self.specific_cost_ex_drilling = 1e3 * (self.cost - self.drilling_costs) / abs(self.net_power_elec * 1e-6)  # $€/kW
...
if __name__ == "__main__":
    ...
    def drilling_sens(drilling_cost):
        Cycle = ORC(recu=False)
        Cycle.deltaT_superheat = 2
        Cycle.condenser.deltaP_cold = 120
        Cycle.coolingpump.eta_isentropic = 0.6

        Cycle.drilling_costs=drilling_cost
        ...
        print(drilling_cost, Cycle.net_power_elec, Cycle.specific_cost_ex_drilling, Cycle.specific_cost)

    for d in [0, 1, 2, 4, 8, 16, 32, 64, 100]:
        drilling_sens(d)

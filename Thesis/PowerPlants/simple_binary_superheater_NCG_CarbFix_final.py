...
class ORC(BinaryCycle):
    ...
    def __calc_NCG_handling(self, gf_out, P_absorb):

        if self.NCG_handled:

            self.ncg_sep_lp.set_inputs(gf_out)
            brine_lp, ncg_lp = self.ncg_sep_lp.calc()

            self.brine_pump_lP.set_inputs(brine_lp, P_absorb)
            brine_mp = self.brine_pump_lP.calc()

            self.ncg_comp_lp.set_inputs(ncg_lp, P_absorb, findN=True)
            ncg_mp = self.ncg_comp_lp.calc()

            self.absorption.set_inputs(brine_mp, ncg_mp)
    ...
    def update_mass_rate(self, m):
        ...
        # NCG handling
        self.ncg_sep_lp.update_inlet_rate(self.geofluid.m)
        lp_brine_m = self.ncg_sep_lp.outlet[0].m
        ...
        self.absorption.update_inlet_rate([lp_brine_m, lp_ncg_m])
    ...
    def calc(self, P_max, T_max, T_min, P_absorb, *args):
        ...
        self.__calc_NCG_handling(geofluid_out, P_absorb)
...
if __name__ == "__main__":
    ...
    workingfluid = Fluid(["Isohexane", 1])
    ...
    START=perf_counter()
    Cycle.calc(Pmax, Tmax, Tmin, 30e5)
    END = perf_counter()

...
class ORC(BinaryCycle):
    ...
    def __init__(self, recu=True):
        ...
        self.feedpump = Simulator.pump(0.85, cost_model="default")

        self.superheater = Simulator.heat_exchanger(deltaT_pinch=self.deltaT_pinch_vap + 0.1,
                                                    cost_model="peters-floating")
        self.evaporator = Simulator.heat_exchanger(deltaT_pinch=self.deltaT_pinch_vap, cost_model="peters-floating")
        self.preheater = Simulator.heat_exchanger(deltaT_pinch=self.deltaT_pinch_liq, cost_model="peters-floating")

        self.temp_HX = Simulator.heat_exchanger(deltaT_pinch=self.deltaT_pinch_liq)

        self.recuperator = Simulator.heat_exchanger(deltaT_pinch=self.deltaT_pinch_vap, cost_model="recuperator")
        ...
        self.turbine = Simulator.turbine(0.85, cost_model="similitude")
    ...
    def __calc_cost(self):
        ...
        self.costs = [{"equip": "Superheater", "val": self.superheater.cost},
                      ...
                      # {"equip": "SecondaryEquip", "val": secondary_equip},
                      # {"equip": "Construction", "val": construction}
                      ]
        if self.recuperated:
